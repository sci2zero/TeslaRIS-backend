...
public interface CommissionReportRepository extends JpaRepository<CommissionReport, Integer> {
    ...
    @Query("SELECT cr.reportFileName FROM CommissionReport cr WHERE cr.commission.id = :commissionId")
    List<String> getAvailableReportsForCommission(Integer commissionId);

    @Query("SELECT cr FROM CommissionReport cr WHERE cr.commission.id = :commissionId AND cr.reportFileName = :reportFileName")
