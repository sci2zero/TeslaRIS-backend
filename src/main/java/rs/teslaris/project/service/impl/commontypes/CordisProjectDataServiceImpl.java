...
public class CordisProjectDataServiceImpl implements CordisProjectDataService {
    ...
    private void populateFromDocument(PrepopulatedProjectMetadataDTO metadata, Document document)
        ...
        var uriNodes = (NodeList) xpath.evaluate(
            "//*[local-name()='webLink'][@represents='project'][@type='relatedWebsite']/*[local-name()='physUrl']",
    ...
    @Nullable
    private OrganisationUnitProjectContributionType mapCordisTypeToContributionType(
        String cordisType) {
    ...
    private ProjectStatus mapCordisStatusToProjectStatus(String cordisStatus) {
        return switch (cordisStatus) {
            ...
            case "TERMINATED" ->
                ProjectStatus.CANCELLED; // CORDIS website could be queried by this status but fetching a project with it results in request time out
            default -> {
