...
public class ResearchAreaControllerTest extends BaseTest {
    ...
    @Order(Integer.MAX_VALUE)
    @WithMockUser(username = "test.admin@test.com", password = "testAdmin")
    public void testGetChildResearchAreas() throws Exception {
        String jwtToken = authenticateAdminAndGetToken();

        mockMvc.perform(
                MockMvcRequestBuilders.get(
                        "http://localhost:8081/api/research-area/children/{researchAreaId}",
    ...
    @Test
    @WithMockUser(username = "test.admin@test.com", password = "testAdmin")
    public void testSearchResearchAreas() throws Exception {
        mockMvc.perform(
            MockMvcRequestBuilders.get(
                    "http://localhost:8081/api/research-area/search?tokens=test&lang=sr&page=0&size=10")
                .contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
    }
}
