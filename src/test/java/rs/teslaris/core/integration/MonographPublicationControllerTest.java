...
public class MonographPublicationControllerTest extends BaseTest {
    ...
    private MonographPublicationDTO getTestPayload() {
        ...
        monographPublicationDTO.setDocumentDate(new FlexibleDateDTO(2004, 11, 6, null));
    ...
    @Test
    @WithMockUser(username = "test.admin@test.com", password = "testAdmin")
    public void testReadMonographPublicationByOldId() throws Exception {
        mockMvc.perform(
            MockMvcRequestBuilders.get(
                    "http://localhost:8081/api/monograph-publication/old-id/{oldId}", 991)
                .contentType(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
    }
}
